...
def compare_snapshots(newer_snapshot_path: str | Path, 
                      older_snapshot_path: str | Path,
                      newer_date: date | None = None,
                      older_date: date | None = None) -> list[Milestone1KRepo]:
...
def _extract_date_from_filename(snapshot_path: str | Path) -> date:
    ...
    if filename.startswith("snapshot_"):
        date_str = filename[len("snapshot_"):]
        return date.fromisoformat(date_str)
    
    raise ValueError(f"无法从文件名 {path.name} 中提取日期")


def send_snapshot_diff_to_wecom(newer_snapshot_path: str | Path,
                                older_snapshot_path: str | Path,
    ...
    if not milestones:
        ...
        message = f"📊 今日没有新增突破1k star的项目\n\n分析范围:\n- 较新快照: {Path(newer_snapshot_path).name}\n- 较旧快照: {Path(older_snapshot_path).name}\n- 共同项目数: {len(set(load_snapshot_data(newer_snapshot_path).keys()) & set(load_snapshot_data(older_snapshot_path).keys()))}"
...
def get_snapshot_diff_summary(newer_snapshot_path: str | Path,
                              older_snapshot_path: str | Path) -> dict[str, Any]:
    """获取快照差异的统计摘要。
...
def find_latest_snapshots(data_dir: str | Path = "data") -> tuple[Path | None, Path | None]:
    ...
    for pattern in ["snapshot_*.json", "snapshot_*.json.gz"]:
        for path in data_path.glob(pattern):
            try:
                ...
                snapshot_files.append((date, path))
            except ValueError:
                continue
    ...
    if len(snapshot_files) >= 2:
        return snapshot_files[0][1], snapshot_files[1][1]
    else:
        return snapshot_files[0][1], None
    ...
    if args.summary:
        ...
        if summary['top_projects']:
            ...
            for project in summary['top_projects']:
                print(f"   {project['rank']}. {project['full_name']} - {project['stars_today']:,} ⭐ (+{project['stars_gained']:,})")
    else:
        # 发送企业微信推送
